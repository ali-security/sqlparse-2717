...
def test_issue35():
    ...
    sql = sqlparse.format("select * from foo where bar = 1 limit 1", reindent=True)
...
def test_issue40():
    ...
    s = sqlparse.format(
        "SELECT id ==  name FROM " "(SELECT id, name FROM bar)", reindent=True
    )
    assert s == "\n".join(
    ...
    s = sqlparse.format(
        "SELECT id ==  name FROM " "(SELECT id, name FROM bar) as foo", reindent=True
    )
    assert s == "\n".join(
        [
            "SELECT id == name",
...
def test_issue90():
    sql = (
        'UPDATE "gallery_photo" SET "owner_id" = 4018, "deleted_at" = NULL,'
...
def test_null_with_as():
    ...
    formatted = sqlparse.format(sql, reindent=True)
    tformatted = "\n".join(["SELECT NULL AS c1,", "       NULL AS c2", "FROM t1"])
    assert formatted == tformatted
...
@pytest.mark.parametrize("s", ["SELECT x AS", "AS"])
...
def test_issue562_tzcasts():
    ...
    assert formatted == "SELECT f(HOUR\n         from bar AT TIME ZONE 'UTC')\nfrom foo"
